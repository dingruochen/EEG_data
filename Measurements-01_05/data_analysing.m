...
% Load data
str = "new_4channels_nobias3.csv"; % Replace with your actual file path
T = readtable(str);             
x = T.Var2;
...
% Find the peak power of 50Hz for the full data set
target_freq_50Hz_full = (freq_full >= 49) & (freq_full <= 51);
[target_peak_power_50Hz_full, target_peak_50Hz_full] = max(psdx_full(target_freq_50Hz_full));
peak_freq_50Hz_full = freq_full(target_freq_50Hz_full);
peak_freq_50Hzfull = peak_freq_50Hz_full(target_peak_50Hz_full); % Corrected frequency
...
fprintf('SSVEP Peak amplitude for the entire data: %.2f dB/Hz\n', pow2db(target_peak_power_full));
fprintf('50Hz Peak amplitude for the entire data: %.2f dB/Hz\n', pow2db(target_peak_power_50Hz_full));
