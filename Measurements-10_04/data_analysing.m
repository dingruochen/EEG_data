...
% Load data
str = "COMP_RES3_10_4.csv"; % Replace with your actual file path
T = readtable(str);             
x = T.Var2;
...
% Give arrays to store SNR values and peak frequencies
SNR_values = zeros(1, number_of_segments);
peak_frequencies = zeros(1, number_of_segments);

% Give array to store peak amplitudes for each segment
...
for k = 1:number_of_segments
    ...
    target_freq_indices = (freq_segment >= 12) & (freq_segment <= 13);
...
target_freq_indices_full = (freq_full >= 12) & (freq_full <= 13);
